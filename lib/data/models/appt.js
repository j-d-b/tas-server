...
},
{
  getterMethods: {
    timeSlot: function() {
      const zonedDate = moment(this.timeSlotDateUTC).tz(TIMEZONE);
      return {
    ...
    arrivalWindow: function() {
      const startDateZoned = moment(this.timeSlotDateUTC).tz(TIMEZONE).add(this.arrivalWindowSlot * this.arrivalWindowLength, 'minutes');
      const endTimeUTC = moment(this.timeSlotDateUTC).tz(TIMEZONE).add((this.arrivalWindowSlot + 1) * this.arrivalWindowLength, 'minutes');
