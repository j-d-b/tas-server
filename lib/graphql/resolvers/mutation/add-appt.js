...
const addAppt = isAuthenticatedResolver.createResolver(
  async (_, { input: { timeSlot, licensePlateNumber, notifyMobileNumber, comment, actions } }, { user, Appt, Action, Config, Restriction }) => {
    ...
    const newActions = await Promise.all(actions.map(async (action) => {
      ...
      if (action.type === 'IMPORT_FULL') typeSpecific.containerSize = 'TWENTY_FOOT'; // TODO
    ...
    await isAvailableCheck(newAppt, newActions, Action, Appt, Config, Restriction); // appt scheduling logic
