...
const addAppt = isAuthenticatedResolver.createResolver(
  async (_, { input: { timeSlot, licensePlateNumber, notifyMobileNumber, comment, actions } }, { user, Appt, Action, Config, Restriction, RestrictionTemplate }) => {
    ...
    if (notifyMobileNumber) { // TODO test this
      const apptDetails = {
        date: getPrettyDateString(newAppt.timeSlot.date),
        arrivalWindow: ((appt) => {
          const getHourString = hourVal => hourVal < 10 ? `0${hourVal}:00` : `${hourVal}:00`;
          const apptDate = moment(`${newAppt.timeSlot.date} ${getHourString(newAppt.timeSlot.hour)}:00`);
          const startDate = apptDate.add(appt.arrivalWindowSlot * appt.arrivalWindowLength, 'minutes');
          const endDate = apptDate.add((appt.arrivalWindowSlot + 1) * appt.arrivalWindowLength, 'minutes');
