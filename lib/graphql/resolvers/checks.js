...
module.exports.doesRestrictionExistCheck = async (restrictionId, Restriction) => {
  const targetRestriction = await Restriction.findById(restrictionId);
...
module.exports.doesRestrictionTemplateExistCheck = async (templateName, RestrictionTemplate) => {
  const targetTemplate = await RestrictionTemplate.findById(templateName);
...
module.exports.isAvailableCheck = async (appt, actions, Action, Appt, Config, Restriction, RestrictionTemplate) => {
  const { timeSlot } = appt;

  const isSlotAvailable = await slotAvailability(timeSlot, Appt, Config, Restriction, RestrictionTemplate);
