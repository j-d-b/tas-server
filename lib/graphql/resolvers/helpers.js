...
module.exports.slotBlockAvailability = async (apptId, timeSlot, numActionsPerBlock, Action, Appt, Block, Restriction) => {
  for (const [blockId, numActions] of Object.entries(numActionsPerBlock)) {
    ...
    const slotBlockPlannedActivities = await Restriction.findOne({ where: { timeSlotHour: timeSlot.hour, timeSlotDate: timeSlot.date } }).then(restriction => restriction ? restriction.plannedActivities : 0);
...
module.exports.slotTotalAvailability = async (timeSlot, Appt, Config, Restriction) => {
  let slotTotalAllowed = await Restriction.findOne({ where: { timeSlotHour: timeSlot.hour, timeSlotDate: timeSlot.date } }).then(restriction => restriction && restriction.gateCapacity);
