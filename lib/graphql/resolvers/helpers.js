...
const { addWeeks, endOfToday, startOfHour, isBefore, format, addHours } = require('date-fns');
...
module.exports.getDateString = ISOString => format(Date.parse(ISOString), 'iii, MMM d, yyyy');
...
const zerosPad = value => value < 10 ? `0${value}` : value;
...
module.exports.getTimeSlotsInNextWeek = () => {
  ...
  while (isBefore(currDate, oneWeekFromToday)) {
    const date = `${currDate.getUTCFullYear()}-${zerosPad(currDate.getUTCMonth() + 1)}-${zerosPad(currDate.getUTCDate())}`;
  ...
  console.log(timeSlotsInNextWeek[0]);
...
module.exports.slotAvailability = async (timeSlot, Appt, Config, Restriction, RestrictionTemplate) => {
  ...
  if (slotCapacity === null) { // no global restriction, check for template restriction
    ...
    if (appliedTemplate) { // applied template restriction
      slotCapacity = await Restriction.findOne({ where: { template: appliedTemplate.name, hour: timeSlotDateUTC.getUTCHours(), dayOfWeek: dayEnumMapping[timeSlotDateUTC.getUTCDay()] } }).then(res => res && res.gateCapacity);
    }
