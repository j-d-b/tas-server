...
const { Appt } = require('../data/models');

const sendApptReminders = async () => {
  ...
  for (const appt of apptsTomorrow) {
    ...
    const apptDetails = {
      name: user.name,
      date: moment(appt.timeSlot.date).format('ddd, MMM Do, YYYY'),
  ...
  logger.info(`${numSentEmail}/${numEmail} email and ${numSentSMS}/${numSMS} SMS notification(s) sent successfully.`);
};
